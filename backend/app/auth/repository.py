...
class UserRepository:
    ...
    def create_user(
        self, email: str, hashed_password: str, full_name: str, telefono: str | None = None
    ) -> User:
        """Create a new user"""
        user = User(
            email=email, hashed_password=hashed_password, full_name=full_name, telefono=telefono
        )
        self.db.add(user)
        self.db.commit()
    ...
    def get_user_by_email(self, email: str) -> User | None:
        """Get user by email"""
        return (
            self.db.query(User).filter(User.email == email, User.soft_deleted_at.is_(None)).first()
        )

    def get_user_by_id(self, user_id: UUID) -> User | None:
        """Get user by ID"""
        return (
            self.db.query(User).filter(User.id == user_id, User.soft_deleted_at.is_(None)).first()
        )
    ...
    def get_user_with_roles(self, user_id: UUID) -> User | None:
    ...
    def count_admin_users(self) -> int:
        ...
        return (
            self.db.query(User)
            .join(UserRole)
            .filter(UserRole.role == "ADMIN", User.soft_deleted_at.is_(None))
            .distinct()
            .count()
        )
