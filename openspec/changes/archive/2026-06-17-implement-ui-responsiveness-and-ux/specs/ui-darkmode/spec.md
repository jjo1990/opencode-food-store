...
Especifica el sistema de dark mode para Food Store. Cubre la estrategia `class` de Tailwind, el ThemeToggle con iconos sun/moon, la persistencia de preferencia en localStorage vía uiStore, la sincronización de la clase `dark` en `<html>`, y la migración de todos los componentes compartidos, páginas y features a variantes `dark:`.
